class Course():
    def __init__(self, id='', title='', instructor='', credits=4, enrolled=0, capacity=0):
        self.id = id
        self.title = title
    ...
    def __str__(self):
        return f'{self.id}  {self.title}  {self.instructor}  {self.credits:.2f}  {self.enrolled}/{self.capacity}'
    
    def add_course(self, number_of_students=1):
        self.enrolled += number_of_students
        print(f'{number_of_students} student(s) joined the course! Now we have {self.enrolled} students.')

    def drop_course(self, number_of_students=1):
        self.enrolled -= number_of_students
        print(f'{number_of_students} student(s) dropped the course! Now we have {self.enrolled} students.')
...
def main():
    ...
    fin = Course('FIN3515', 'Corporate Financial Management', 'Laurie Krigman', 4, 42, 38)
    print(mis)
    print(fin)
